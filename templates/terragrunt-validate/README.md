...
Validates Terraform files using `terragrunt validate`.
...
jobs:
- template: templates/terragrunt-validate/main.yaml@templates
  parameters:
    pool: 
...
| tf_validate_command_options[^1]   | Options to pass into the terraform validate command.                            |  `none`  |
